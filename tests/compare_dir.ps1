...
# compare ptpng vs libpng over a directory (default: generated matrix)
$dir = if ($args[0]) { $args[0] } else { "tests\gen" }
...
foreach ($tag in $formats) {
    ...
    foreach ($f in Get-ChildItem $dir -Filter *.png) {
        ...
        if ($diff) { $fail++; Write-Output "MISMATCH $($f.Name) $tag ($diff bytes)" }
        else { $pass++ }
    }
}
"pass=$pass fail=$fail"
